...
Pipeline::~Pipeline()
{
    ...
    vkDestroyDescriptorSetLayout(
      device_->device(), descriptorSetLayout_, nullptr);
...
Pipeline::create_shader_module_()
{
    ...
    return vkCreateShaderModule(
      device_->device(), &createInfo, nullptr, &module_);
}
...
Pipeline::create_descriptor_set_()
{
    ...
    auto ret = vkCreateDescriptorPool(
      device_->device(), &createInfo, nullptr, &descriptorPool_);
    if (ret != VK_SUCCESS) {
        return ret;
    }
    ...
    ret =
      vkAllocateDescriptorSets(device_->device(), &allocInfo, &descriptorSet_);
    if (ret != VK_SUCCESS) {
        return ret;
    }
...
Pipeline::create_query_pool_()
{
    ...
    return vkCreateQueryPool(
      device_->device(), &createInfo, nullptr, &queryPool_);
}
...
Pipeline::set_bindings_(std::vector<VkTensor> bindings)
{
    ...
    vkUpdateDescriptorSets(
      device_->device(), writes.size(), writes.data(), 0, nullptr);
    return VK_SUCCESS;
}
...
Pipeline::time()
{
    ...
    vkGetQueryPoolResults(device_->device(),
                          queryPool_,
                          0,
                          2,
