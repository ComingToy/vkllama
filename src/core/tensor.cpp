...
VkTensor::VkTensor()
    : c_(0), h_(0), w_(0), dev_(nullptr), visable_(false), data_(VK_NULL_HANDLE), status_(nullptr)
{
...
VkTensor::VkTensor(const int c,
                   ...
                   GPUDevice* dev,
                   const bool visable)
    : c_(c), h_(h), w_(w), dev_(dev), visable_(visable), data_(VK_NULL_HANDLE), status_(nullptr)
{
    mem_ = {VK_NULL_HANDLE, 0, 0, 0, nullptr, 0};
}

VkTensor::VkTensor(const VkTensor& rhs)
    : c_(rhs.channels()), h_(rhs.height()), w_(rhs.width()), dev_(rhs.dev_), visable_(rhs.visable()), data_(rhs.data_), mem_(rhs.mem_), status_(rhs.status_)
{
    if (status_)
    {
...
VkTensor::VkTensor(VkTensor&& rhs)
    : c_(rhs.channels()), h_(rhs.height()), w_(rhs.width()), dev_(rhs.dev_), visable_(rhs.visable_), data_(rhs.data_), mem_(rhs.mem_), status_(rhs.status_)
{
    rhs.status_ = nullptr;
}
...
void VkTensor::set_access_flags(VkAccessFlags flags)
{
    if (!status_)
        return;
...
void VkTensor::set_pipeline_stage(VkPipelineStageFlags stage)
{
    if (!status_)
        return;
