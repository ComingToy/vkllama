...
public:
  ...
  VkResult allocate (VkMemoryRequirements const &req, MemBlock *block,
                     bool visable = false);
...
private:
  ...
  VkResult allocate_ (VkMemoryRequirements const &req,
                      const VkMemoryPropertyFlags flags, __MemBlock &block);
  VkResult allocate_from_pool_ (const uint32_t type, const VkDeviceSize size,
                                const VkDeviceSize align, __MemBlock &result);

  void merge_block_ (const uint32_t type);
