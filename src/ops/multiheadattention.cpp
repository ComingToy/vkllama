...
MultiHeadAttention::init () noexcept
{
  ...
  out_matmul_ = std::make_unique<MatMul> (dev_, command_, wo_, 1.0, .0, 0, 0,
  ...
  for (size_t i = 0; i < wq_.size (); ++i)
    {
      auto k_matmul = std::make_unique<MatMul> (
          dev_, command_, wk_[i], 1.0, .0, 0, 0, transposed_weight_, dtype_);
      auto q_matmul = std::make_unique<MatMul> (
          dev_, command_, wq_[i], 1.0, .0, 0, 0, transposed_weight_, dtype_);
      auto v_matmul = std::make_unique<MatMul> (
          dev_, command_, wv_[i], 1.0, .0, 0, 0, transposed_weight_, dtype_);
      auto weighted_matmul = std::make_unique<MatMul> (dev_, command_, 1.0, .0,
                                                       0, 0, 0, dtype_);

      auto rope
          = std::make_unique<Rope> (dev_, command_, maxlen_, dim_, dtype_);
      auto attn_score = std::make_unique<MatMul> (dev_, command_, 1.0, .0, 0,
                                                  0, 1, dtype_);
      auto elementwise
      ...
      if (use_kvcache_)
        {
          ...
          ret = update_kcache_op (kcache_, k, (uint32_t)offset);
          if (ret != VK_SUCCESS)
            {
              return ret;
            }

          ret = update_vcache_op (vcache_, v, (uint32_t)offset);
          if (ret != VK_SUCCESS)
            {
              return ret;
            }
