    ...
    : Op(dev, command), w1_(w1), w2_(w2), w3_(w3)
{
    ...
    pipeline0_.reset(new Pipeline(dev_, __get_matmul_shared_mem_comp_spv_code(),
                                  __get_matmul_shared_mem_comp_spv_size(),
                                  {act}, shaderInfo));
    shaderInfo.specialization_count = 0;
    pipeline1_.reset(new Pipeline(dev_, __get_matmul_shared_mem_comp_spv_code(),
                                  __get_matmul_shared_mem_comp_spv_size(), {},
                                  shaderInfo));
    pipeline2_.reset(new Pipeline(dev_, __get_matmul_shared_mem_comp_spv_code(),
                                  __get_matmul_shared_mem_comp_spv_size(), {},
                                  shaderInfo));
    ...
    pipeline3_.reset(new Pipeline(dev_, __get_element_wise_comp_spv_code(),
                                  __get_element_wise_comp_spv_size(), {op_type},
                                  shaderInfo));
}

VkResult FeedForward::init()
{
...
uint64_t FeedForward::time()
{
    return std::max(pipeline0_->time(), pipeline2_->time()) + pipeline1_->time() + pipeline3_->time();
}

VkResult FeedForward::operator()(VkTensor X, VkTensor& output)
{
