...
FeedForward::FeedForward(GPUDevice* dev,
                         Command* command,
                         VkTensor w1,
                         VkTensor w2,
                         VkTensor w3)
  ...
  , w3_(w3)
{
    ...
    Pipeline::ConstantType op_type = { .u32 = 2 };
    pipeline3_.reset(new Pipeline(dev_,
...
FeedForward::operator()(VkTensor X, VkTensor& output)
{
    ...
    output = VkTensor(t2_.channels(), t2_.height(), w2_.width(), dev_);
    ...
    command_->record_pipeline(*pipeline3_, { t0_, t1_, t2_ }, { N });
