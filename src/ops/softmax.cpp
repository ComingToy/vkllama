...
Softmax::init ()
{
  ...
  Pipeline::ShaderInfo info1 = { 0, 2, 4, 32, 4, 1 };
...
Softmax::operator() (VkTensor a, VkTensor &b)
{
  ...
  ret = command_->record_pipeline (*softmax0_, { a, bias_, m_, b }, shape);
  if (ret != VK_SUCCESS)
    {
      return ret;
    }
  ...
  ret = command_->record_pipeline (*softmax1_, { m_, b }, shape);
  if (ret != VK_SUCCESS)
    {
      return ret;
    }
