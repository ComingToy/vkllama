...
TEST_P (TestMatmul, test_matmul_broadcast)
{
  ...
  VkTensor output;
  ASSERT_TRUE (matmul_op (input0->first, input1->first, output) == VK_SUCCESS)
      << "failed at forwarding matmul op";

  std::vector<float> buf (output.size ());
  ASSERT_EQ (command_->download (output, buf.data (), buf.size ()), VK_SUCCESS)
  ...
  Eigen::Tensor<float, 0, Eigen::RowMajor> mse
      = (eigen_output - output_mapped).pow (2.0f).mean ();
  ASSERT_LT (*mse.data (), 1e-4f);
}

std::vector<TestMatMulParams> params = {
#if 1
  { 1, 1024, 1023, 225, 0, 1 }, { 1, 1027, 619, 32, 0, 1 },
  { 5, 1024, 512, 256, 0, 1 },  { 16, 255, 321, 513, 0, 1 },
  { 1, 1024, 1023, 225, 1, 1 }, { 1, 1027, 619, 32, 1, 1 },
  { 5, 1024, 512, 256, 1, 1 },  { 16, 255, 321, 513, 1, 1 },
  { 1, 1024, 1023, 225, 2, 1 }, { 1, 1027, 619, 32, 2, 1 },
  { 5, 1024, 512, 256, 2, 1 },  { 16, 255, 321, 513, 2, 1 },

  { 1, 1024, 1023, 225, 0, 0 }, { 1, 1027, 619, 32, 0, 0 },
  { 5, 1024, 512, 256, 0, 0 },  { 16, 255, 321, 513, 0, 0 },
  { 1, 1024, 1023, 225, 1, 0 }, { 1, 1027, 619, 32, 1, 0 },
  { 5, 1024, 512, 256, 1, 0 },  { 16, 255, 321, 513, 1, 0 },
  { 1, 1024, 1023, 225, 2, 0 }, { 1, 1027, 619, 32, 2, 0 },
  { 5, 1024, 512, 256, 2, 0 },  { 16, 255, 321, 513, 2, 0 },
#else
  { 5, 1024, 512, 256, 1, 1 }
#endif
};
